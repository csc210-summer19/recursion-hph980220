...
public class LinkedList<E extends Comparable<E>> {
	...
	private void helpRemoveAll(Node ref, E el) {
		...
		if (ref.next.next == null && !ref.next.data.equals(el)) {
			return;
		}
		if (ref.next.next == null && ref.next.data.equals(el)) {
			ref.next = null;
...
// Duplicate el next to each occurrence of el in this list.
	public void duplicateAll(E el) {
		// This public method requires a call to a private helper method
		// with first as an argument. It must be recursive, no loop allowed.
		helpDuplicateAll(first, el);
	}

	private void helpDuplicateAll(LinkedList<E>.Node ref, E el) {
		// TODO Auto-generated method stub
		if (first == null)
			return;
		if (ref.next == null) {
		...
		if (ref.data.equals(el)) {
			...
			ref.next = newel;
			newel.next = temp;
			helpDuplicateAll(newel.next, el);
			n++;
		} else {
			helpDuplicateAll(ref.next, el);
		}
	}
